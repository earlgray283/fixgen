...
func (tq *TodoQuery) QueryUsers() *UserQuery {
...
func (tq *TodoQuery) WithUsers(opts ...func(*UserQuery)) *TodoQuery {
	query := (&UserClient{config: tq.config}).Query()
...
func (tq *TodoQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Todo, error) {
	...
	if query := tq.withUsers; query != nil {
		if err := tq.loadUsers(ctx, query, nodes, nil,
			func(n *Todo, e *User) { n.Edges.Users = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TodoQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Todo, init func(*Todo), assign func(*Todo, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Todo)
	for i := range nodes {
